...
Public Class Twitter
    ...
    Public Sub Initialize(ByVal token As String, ByVal tokenSecret As String, ByVal username As String, ByVal userId As Long)
        ...
        Try
            ...
            Dim stream2 As New FileStream("session", FileMode.Open, FileAccess.Read, FileShare.Read)
            keitaiWeb.session = CType(formatter2.Deserialize(stream2), Session)
            stream2.Close()
        Catch ex As Exception
            MessageBox.Show("Keitai Webにログインできませんでした。")
            SignInForm.ShowDialog(TweenMain)
        End Try
    End Sub
    ...
    Public Function PostStatus(ByVal postStr As String, ByVal reply_to As Long) As String
        ...
        Select Case res
            ...
            Case HttpStatusCode.Forbidden, HttpStatusCode.BadRequest
                ...
                ElseIf errMsg = "User is over daily status update limit." Then
                    Try
                        keitaiWeb.Post(postStr, reply_to)
                        Return ""
                    Catch ex As Exception
                        'Dim rmsr As DialogResult = MessageBox.Show("なんかえらー。投稿できなかったよ", "Keitai Web", MessageBoxButtons.YesNo)
                        Return PostStatus(postStr & "　", reply_to)
                        'Return "Warn:" + errMsg
                    End Try
                ElseIf errMsg = "Status is a duplicate." Then
                    Return PostStatus(postStr & "　", reply_to)
                Else
                    Return "Warn:" + errMsg
                End If
    ...
    Public Function PostRetweet(ByVal id As Long, ByVal read As Boolean) As String
        ...
        Select Case res
            ...
            Case HttpStatusCode.Forbidden
                'Dim errMsg As String = GetErrorMessageJson(content)
                'If res.ToString() = "User is over daily status update limit." Then
                Try
                    keitaiWeb.ReTweet(target, TabInformations.GetInstance.Item(id).ScreenName)
                    Return ""
                Catch ex As Exception
                    MessageBox.Show("なんかえらー。RTできなかったよ")
                End Try
                'End If
